...
def secure_hash(password: str, username: str) -> str:
    """Uses standard-library PBKDF2 key stretching to eliminate brute-force risk securely."""
                ...
                if u_input and p_input and reg_matric:
                    ...
                    if cursor.fetchone():
                        ...
                        try:
                            ...
                            st.success("Secure account verified!")
                            st.rerun()
                        except sqlite3.IntegrityError:
                            st.error(f"❌ Username '{u_input}' is unavailable.")
                            suggestions = []
                            base_clean = u_input.lower().replace(" ", "")
                            candidate_patterns = [f"{base_clean}{random.randint(10, 99)}", f"{base_clean}_fps", f"{base_clean}_student"]
                            ...
                            st.markdown("**💡 Try an available variant:**")
                            for sugg in suggestions[:2]: st.code(sugg)
                        finally:
                            conn.close()
                else:
                    st.warning("Please complete all registration entry blanks.")
        else:
